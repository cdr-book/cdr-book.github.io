...
[^Gartner]: Véase “Gartner Identifies Four Methods for Measuring Marketing’s Impact)”, disponible en https://www.gartner.com/en/newsroom/press-releases/2020-03-04-gartner-identifies-four-methods-for-measuring-marketi

\index{Return On Investment, ROI}

1. **_Marketing Mix Modeling_ (MMM)**\index{Marketing Mix Modeling, MMM}: modelos de series temporales que sirven para estimar la contribución del marketing u otras variables explicativas a las ventas desde un punto de vista estratégico. \index{marketing}

2. **_Multitouch Attribution_ (MTA)**\index{Multitouch Attribution, MTA}: modelos de atribución que valoran cada punto de contacto (*touchpoint*) del recorrido del cliente (*customer journey*), asignando a cada uno un peso en la conversión (venta, descarga de folleto, etc). Son modelos tácticos que normalmente se centran en el canal online. 

3. **_Holdout Testing o Experiments_ (EXP)**\index{Holdout Testing o Experiments, EXP}: modelos causales que evalúan el impacto de una campaña publicitaria a partir de una muestra de control y otra de test. 

4. **_Unified Measurement Approaches_ (UMA)**\index{Unified Measurement Approaches, UMA}: combinación de los modelos anteriores (MMM + MTA + EXP) con el objetivo de tener una visión unificada de los resultados.
...
`Robyn` está ganando en popularidad debido a las continuas mejoras que están aplicando desde Meta, así como gracias a su adaptabilidad a la realidad del anunciante. Además, existe un paquete en **R** validado y subido al CRAN que lo sitúa como una gran alternativa de código abierto en **R** para iniciarse en el campo de la medición y optimización del retorno de las inversiones publicitarias. La ecuación empleada es la siguiente:
\begin{equation}
...
\alpha,\gamma &= \text{hiperparámetros que definen la Scurve}, \\
...
y se ejecuta el modelo definiendo el número de iteraciones y *trials*. En este ejercicio, el número de iteraciones será 2000 y el de *trials* 10, obteniendo un total de 20.000 posibles soluciones del modelo:
