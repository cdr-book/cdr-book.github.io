...
{\centering \includegraphics[width=0.8\linewidth]{./img/efectos} 

}

\caption{Cuestiones para determinar si un efecto es fijo o aleatorio.}(\#fig:tab1)
\end{figure}
...
-   $\bf{X}$ es una matriz $n \times k$ ($k$ es el número de efectos fijos).
-   $\bf {Z}$ es una matriz $n \times p$ ($p$ es el número de efectos
    aleatorios).
...
{\centering \includegraphics[width=0.6\linewidth]{./img/rectas1} 

}

\caption{Ilustración de posibles escenarios para dos escuelas.}(\#fig:rectas1)
\end{figure}
...
\caption{Ilustración de posibles escenarios para tres escuelas.}(\#fig:rectas2)
\end{figure}

El modelo que permite tener en cuenta esta situación es:
$$y_{ij} = \beta_{0i} + \beta_{1i}x_{ij} + \epsilon_{ij},$$
donde aparece ahora el subíndice $i$ también en la pendiente, lo que indica que cada centro
...
{\centering \includegraphics[width=0.6\linewidth]{150019_mxm_files/figure-latex/plot-qqmath-1} 

}

\caption{ Efectos aleatorios junto con  sus intervalos de confianza.}(\#fig:plot-qqmath)
\end{figure}
