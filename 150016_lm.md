...
- $\beta_0$ es el **término independiente**\index{término independiente} o **constante** del modelo,
- $\beta_1, \ldots, \beta_p$ son los **coeficientes de regresión**\index{coeficientes de regresión} o **parámetros** del modelo, que se estimarán a partir de los datos observados $(x_{1j}, \ldots, x_{p_j},y_j)$ y reflejan la magnitud del efecto *lineal* (constante) sobre la variable explicada $Y$ de incrementos unitarios en las variables explicativas $X_i$. 
- y $\epsilon_j$ es el **término de error** del modelo\index{error!del modelo}, la parte de $Y$ que no es capaz de explicar la parte determinista del mismo ($\beta_0 + \beta_1 X_{1j} + \ldots + \beta_p X_{pj}$), que se supone sigue una distribución de probabilidad Normal, con media 0 y varianza constante $\sigma^2$;
...
Los datos recogidos u observados sirven para **especificar** la relación predefinida de antemano, mediante la **estimación** de los coeficientes $\beta_i$ que mejor ajustan dicha relación, utilizando el método de **mínimos cuadrados**\index{método!de mínimos cuadrados}. 
...
En la práctica, el coeficiente estimado es *significativo* si su **p-valor**\index{p-valor} (definido en la Sec. \@ref(contrhip)) asociado es suficientemente pequeño.
...
$$e_j = y_j - \hat{y}_j = y_j - (\hat{\beta}_0 +\hat{\beta}_1 x_{1j} + \ldots + \hat{\beta}_p x_{pj}), \hspace{0,2cm} j = 1, \ldots, N.$$ 
...
La utilidad del modelo estimado (especificado) queda plasmada en su utilización para **predecir**\index{predicción del modelo lineal} nuevos valores, $\hat{y_j}$, a partir del conocimiento/asignación de nuevos valores de las variables explicativas, $\{x_{1j}, \ldots, x_{pj}\}$. No obstante, dichas predicciones son valores *esperados* (*medios*), pudiéndose construir sus correspondientes intervalos de confianza.


## Procedimiento con **R**: la función `lm()`

**R** tiene implementada la función `lm()` \index{lm} para ajustar/estimar modelos de regresión lineal múltiple: 
...
A partir de los datos disponibles de $Y, X1, ..., Xp$, la función `lm()` estima los coeficientes $\hat{\beta}_i$ asociados a cada variable $X_i$, mediante el **método de mínimos cuadrados**\index{método!de mínimos cuadrados}, y calcula sus errores estándar, con los que obtiene sus estadísticos de contraste (de la $t$ de Student)^[Con los errores estándar también se pueden obtener los intervalos de confianza de los coeficientes.] y su significación. En el objeto `lm` que se genera también se almacenan los valores ajustados, residuos, etc., que se pueden mostrar a través de funciones genéricas disponibles en **R**. Algunas de ellas son:
...
Cada gráfico muestra un **diagrama de dispersión**\index{diagrama!de dispersión} de sólo dos variables, la explicativa en el eje $X$ y la respuesta en el eje $Y$, obteniéndose la popularmente denominada **nube de puntos**. 
...
El término independiente aparece como `(Intercept)` y toma el valor $\beta_0 =$ -64.3421, el coeficiente asociado a `Solar.R` es $\beta_1 =$ 0.0598, etc. 
...
<div class="figure" style="text-align: center">
<img src="150016_lm_files/figure-html/lm-histogramas-1.png" alt="Histogramas de las variables `Ozone` y `log(Ozone)`" width="60%" />
...
<div class="figure" style="text-align: center">
<img src="150016_lm_files/figure-html/lm-airQpairs-1.png" alt="Gráfico de dispersión por pares de las variables explicativas" width="60%" />
...
El mínimo valor de VIF es 1, no existiendo límite superior. Una regla general para interpretar los VIF es la siguiente: Si el VIF de una variable explicativa $Xi$ es 1, no hay correlación entre ella y cualquier otra variable explicativa del modelo. Si está entre 1 y 5 la correlación es moderada y no provoca graves problemas. Si es mayor que 5 la correlación es fuerte y, probablemente, las estimaciones de los coeficientes y los p-valores resultantes de la estimación del modelo no sean confiables.
...
Se define `Temp_f` dicotomizando `Temp` (tomando sólo dos valores): 1, si la temperatura está por encima de su mediana; y 0, si está por debajo.
...
la concentración media de ozono de los días con temperaturas por encima de la mediana (categoría 1) es
 0.6999 ppb mayor que la de los días con temperaturas inferiores a ella (categoría 0). 
...
En realidad, al incluir un *factor*  en el modelo se está realizando un contraste *t de Student* para averiguar si existen diferencias entre la media de la variable respuesta para cada categoría con respecto a la categoría de referencia. 
