...
{\centering \includegraphics[width=0.6\linewidth]{150017_glm_files/figure-latex/glm-dispersionYlogit-1} 

}

\caption{Gráfico de dispersión del diagnóstico frente a la edad para el ejemplo de enfermedad coronaria (izquierda) y función logística (derecha).}(\#fig:glm-dispersionYlogit)
\end{figure}
...
Como se menciona en el Cap. \@ref(cap-lm), un coeficiente es **significativo** cuando su *p*--valor\index{p-valor@\textit{p} valor} asociado es lo **suficientemente pequeño** (como norma general, inferior a 0,05). No obstante, pueden tomarse otros valores de referencia; por ejemplo, en las salidas de **R** aparecen otros tres niveles de referencia, 0,1, 0,01 y 0,001. 
:::
...
(\#eq:modds1)
\end{equation}
La interpretación de los $\hat{\beta}_i$ carece de sentido. Lo interpretable son los $e^{\hat\beta_i}$, denominados ***odds ratios*** (OR). \index{odds@\textit{odds} ratio} 
...
El modelo de regresión logística se acompañar del denominado **riesgo relativo**\index{riesgo relativo} (RR, _relative risk_). Es una razón de
...
## Regresión de Poisson\index{regresión!de Poisson}
...
{\centering \includegraphics[width=0.6\linewidth]{150017_glm_files/figure-latex/glm-roc-1} 

}

\caption{Gráfico de sensibilidad y especificidad según puntos de corte de discriminación (izquierda) y curva ROC (derecha) para el segundo modelo de regresión logística.}(\#fig:glm-roc)
\end{figure}
...
{\centering \includegraphics[width=0.6\linewidth]{150017_glm_files/figure-latex/glm-grafpois-1} 

}

\caption{Gráfico de barras de $dhosp$ según tipo de diagnóstico (izquierda) y gráficos de cajas de $edad$ según el número de días de hospitalización (derecha).}(\#fig:glm-grafpois)
\end{figure}
...
{\centering \includegraphics[width=0.5\linewidth]{150017_glm_files/figure-latex/glm-predictPois-1} 

}

\caption{Predicciones de todo el conjunto de datos del modelo $regpois$ (izquierda) y $regpois2$ (derecha).}(\#fig:glm-predictPois)
\end{figure}
