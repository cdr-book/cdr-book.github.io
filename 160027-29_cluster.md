...
{\centering \includegraphics[width=0.5\linewidth]{160027-29_cluster_files/figure-latex/cluster-sim-ch27-1} 

}

\caption{Datos simulados que presentan clústeres.}(\#fig:cluster-sim-ch27)
\end{figure}
...
Con variables cualitativas, la similitud entre dos elementos, $s({\bf x}_i;{\bf{x}}_{j})$, es una función que 
...
{\centering \includegraphics[width=0.6\linewidth]{160027-29_cluster_files/figure-latex/plot-dist-euclidea-1} 

}

\caption{$Heatmap$ de  distancias euclídeas.}(\#fig:plot-dist-euclidea)
\end{figure}
...
\index{método! del vecino más lejano}
...
{\centering \includegraphics[width=0.75\linewidth]{160027-29_cluster_files/figure-latex/cluster-compara-1} 

}

\caption{Clusterización jerárquica con distancias euclídeas (dendrograma): métodos del vecino más cercano, vecino más lejano y distancia media.}(\#fig:cluster-compara)
\end{figure}
...
La Fig. \@ref(fig:cluster-compara2), un tanglegrama o diagrama de laberinto, muestra las agrupaciones producidas por los métodos del centroide y la mediana. En ella se puede observar como el método de la mediana corrige la limitación del método del centroide.  \index{tanglegrama}  \index{método! de la mediana} \index{método! del centroide}
...
{\centering \includegraphics[width=0.6\linewidth]{160027-29_cluster_files/figure-latex/cluster-compara2-1} 

}

\caption{Clusterización jerárquica con distancias euclídeas (tanglegrama): método del centroide vs. método de la mediana.}(\#fig:cluster-compara2)
\end{figure}

-   **Método de Ward**  

El método de Ward agrupa, en cada etapa, los dos clústeres que producen el menor incremento de la varianza total intraclúster: $W=\sum_g\sum_{i \in g} (x_{ig}- \bar{x}_g)^{\prime} (x_{ig}- \bar{x}_g)$, donde $\bar{x}_g$ es el centroide del grupo $g$. Así, los grupos formados no distorsionan los datos originales.[^cluster-5]  \index{varianza! intraclúster}
...
{\centering \includegraphics[width=0.6\linewidth]{160027-29_cluster_files/figure-latex/plot-igraph-1} 

}

\caption{Clusterización jerárquica con distancias euclídeas al cuadrado (filograma): método de Ward.}(\#fig:plot-igraph)
\end{figure}
\index{método! de Ward}
-   **Método del encadenamiento intragrupos** \index{método! del encadenamiento intragrupos}
...
{\centering \includegraphics[width=0.6\linewidth]{160027-29_cluster_files/figure-latex/diana-1} 

}

\caption{Clusterización jerárquica divisiva con DIANA.}(\#fig:diana)
\end{figure}
...
Table:(\#tab:cluster-tab-cont) Ejemplo de tabla de contingencia 2 $\times$ 2
 \index{método! del análisis de la asociación}
...
\index{método! del detector automático de interacciones}
...
\index{criterio del gap@criterio del \textit{gap}}
...
{\centering \includegraphics[width=0.7\linewidth]{160027-29_cluster_files/figure-latex/plot-kresults-hc-1} 

}

\caption{Métodos heurísticos para la determinación del número óptimo de clústeres.}(\#fig:plot-kresults-hc)
\end{figure}
...
\index{técnicas de agrupación!no jerárquicas!de clusterización @\textit{clusterización} difusa}
\index{técnicas de agrupación!no jerárquicas!basadas en mixturas de modelos}
\index{técnicas de agrupación!no jerárquicas!Q-factorial}
\index{técnicas de agrupación!no jerárquicas!R-factorial}
\index{técnicas de agrupación!no jerárquicas!directas}
\index{técnicas de agrupación!no jerárquicas!block-clustering @\textit{block-clustering}}
\index{técnicas de agrupación!no jerárquicas!two-mode clustering @\textit{two-mode clustering}}
...
\index{Forgy} \index{k-medias@\textit{k}-medias}
...
{\centering \includegraphics[width=0.5\linewidth]{160027-29_cluster_files/figure-latex/kmeans-1} 

}

\caption{Clusterización no jerárquica con $k$-medias.}(\#fig:kmeans)
\end{figure}
...
\index{gráfico de silueta} \index{número óptimo de clústeres}
...
{\centering \includegraphics[width=0.5\linewidth]{160027-29_cluster_files/figure-latex/pam-1} 

}

\caption{Clusterización no jerárquica con PAM.}(\#fig:pam)
\end{figure}
...
{\centering \includegraphics[width=0.5\linewidth]{160027-29_cluster_files/figure-latex/clara-1} 

}

\caption{Clusterización no jerárquica con CLARA.}(\#fig:clara)
\end{figure}
...
{\centering \includegraphics[width=0.6\linewidth]{160027-29_cluster_files/figure-latex/density-kmeans-1} 

}

\caption{Comparación entre los algoritmos $k$-means y DBSCAN para el conjunto de datos simulado $multishapes$.}(\#fig:density-kmeans)
\end{figure}
\index{técnicas de agrupación!no jerárquicas!directas}
\index{técnicas de agrupación!no jerárquicas!bi-clustering@\textit{bi-clustering}}
\index{técnicas de agrupación!no jerárquicas!co-clustering 
@\textit{co-clustering}}
\index{técnicas de agrupación!no jerárquicas!two-mode clustering @\textit{two-mode clustering}}
\index{técnicas de agrupación!no jerárquicas!block-clustering @\textit{block-clustering}}
\index{técnicas de agrupación!no jerárquicas!de reducción de la dimensionalidad} 
...
\index{técnicas de agrupación!no jerárquicas!clustering @\textit{clustering} difuso} 
...
\index{técnicas de agrupación!no jerárquicas!clustering @\textit{clustering} basado en mixturas de modelos}
